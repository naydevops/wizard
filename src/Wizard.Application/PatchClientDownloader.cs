...
public sealed class PatchClientDownloader
{
    ...
    {
        using var response = await _httpClient.SendAsync(new HttpRequestMessage(HttpMethod.Head, $"LatestBuild/Data/GameData/{fileName}"));
        
        return response is { IsSuccessStatusCode: true, Content.Headers.ContentLength: not null } ? response.Content.Headers.ContentLength.Value : 0;
    }
    ...
    public async Task DownloadFileAsync(string fileName, string filePath)
    {
        ...
        var response =
            await _httpClient.GetAsync($"LatestBuild/Data/GameData/{fileName}", HttpCompletionOption.ResponseHeadersRead);
